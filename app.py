...
def color_schedule(val):
    if isinstance(val, str):
        if "בבית (חופשה)" in val:
            ...
            return 'background-color: #bee5eb; color: #0c5460;'
    return ''

# --- אתחול בסיס נתונים (עם לוחם 1,2,3) ---
...
if 'dept_min_forces' not in st.session_state:
    st.session_state['dept_min_forces'] = {1: 8, 2: 8, 3: 8}
...
def commander_page():
            ...
            if dept_soldiers:
                ...
                with col_btn2:
                    if st.button("🗑️ מחק חייל מהמחלקה", type="primary"):
                        ...
                        st.success(f"{chosen_soldier} נמחק בהצלחה!")
                        st.rerun()
    ...
    with tab3:
        ...
        # --- מנוע ה-CP-SAT המשולב ---
        if st.button(f"🚀 הפעל מנוע אופטימיזציה (CP-SAT)"):
            with st.spinner(f"מפעיל מנוע חקר ביצועים וסורק פתרונות אפשריים..."):
                ...
                num_soldiers = len(dept_names)
                num_days = len(date_list_cols)
                
                # --- תחילת מודל האופטימיזציה ---
                ...
                for i, name in enumerate(dept_names):
                    ...
                    if c.get("סטטוס") == "אושר" and "אין לי העדפה" not in c.get("סוג האילוץ", ""):
                        ...
                        for j, (c_date, day_str) in enumerate(date_list_cols):
                            if c_start and c_end and c_start <= c_date <= c_end:
                                model.Add(X[(i, j)] == 0)
                
                # אילוץ סד"כ
                for j in range(num_days):
                    model.Add(sum(X[(i, j)] for i in range(num_soldiers)) >= req_min_forces)
                
                # פונקציית מטרה משולבת: הוגנות + מקסימום יציאות הביתה
                S_i = [sum(X[(i, j)] for j in range(num_days)) for i in range(num_soldiers)]
                max_S = model.NewIntVar(0, num_days, 'max_S')
                min_S = model.NewIntVar(0, num_days, 'min_S')
                
                for s in S_i:
                    model.Add(max_S >= s)
                    model.Add(min_S <= s)
                
                # הוספת משתנה שסוכם את כל הימים של כולם בבסיס
                total_days_on_base = sum(X[(i, j)] for i in range(num_soldiers) for j in range(num_days))
                
                # כופלים את הפער ב-100 כדי לתת לו עדיפות עליונה, ואז ממזערים גם את סך הנוכחות
                model.Minimize((max_S - min_S) * 100 + total_days_on_base)
                ...
                if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
                    st.success(f"השיבוץ הושלם בהצלחה! זמן ריצת האלגוריתם: {execution_time:.3f} שניות.")
                    
                    schedule_dict = {name: {} for name in dept_names}
                    for i, name in enumerate(dept_names):
                        for j, (_, day_str) in enumerate(date_list_cols):
                            if solver.Value(X[(i, j)]) == 1:
                                schedule_dict[name][day_str] = "נוכח בבסיס"
                            else:
                                schedule_dict[name][day_str] = "בבית (חופשה)"
                    ...
                    # אין צורך ב-rerun כאן, זה יתרנדר אוטומטית למטה
                else:
                    st.error("האלגוריתם לא מצא פתרון אפשרי שעומד בכל האילוצים. נסה להוריד את דרישת הסד\"כ או לשחרר אילוצים של חיילים.")
        ...
        if sched_key in st.session_state['dept_schedules']:
            ...
            st.markdown("### עריכת שיבוץ:")
            
            edited_df = st.data_editor(
                df_to_edit.drop(columns=['מס"ד']), 
                use_container_width=True,
                num_rows="fixed",
                key=f"editor_dept_{selected_dept}_fixed"
            )
            st.session_state['dept_schedules'][sched_key] = edited_df

            st.markdown("### תצוגה צבעונית (קריאה בלבד):")
            display_df = edited_df.copy()
            display_df.insert(0, 'מס"ד', range(1, len(display_df) + 1))
            st.dataframe(display_df.style.map(color_schedule), hide_index=True, use_container_width=True)

            st.markdown("#### 🧮 מחשבון בקרה מבצעי למחלקה:")
            st.markdown(f"**בקרת עמידה ביעד סד\"כ (מינימום נדרש: {req_min_forces}):**")
            dynamic_cols = [c for c in edited_df.columns if c not in ['שם החייל', 'תפקיד / פק"ל']]
            ...
            st.markdown("---")
            st.markdown("### 📊 מדדי ביצוע (KPIs) - ניתוח סיבוכיות תיאורטית ומעשית")
            if st.button("📈 הפק גרף זמני ריצה (10, 50, 100 לוחמים)"):
                with st.spinner("מריץ סימולציות עומס למדידת ביצועי CP-SAT..."):
                    sizes = [10, 50, 100]
                    times = []
                    
                    for size in sizes:
                        sim_model = cp_model.CpModel()
                        sim_X = {}
                        num_sim_days = 14
                        min_forces_sim = int(size * 0.4) 
                        
                        for i in range(size):
                            for j in range(num_sim_days):
                                sim_X[(i, j)] = sim_model.NewBoolVar(f'sim_{i}_{j}')
                                
                        for j in range(num_sim_days):
                            sim_model.Add(sum(sim_X[(i, j)] for i in range(size)) >= min_forces_sim)
                            
                        # הוגנות + מקסימום יציאות הביתה (בסימולציה)
                        sim_S = [sum(sim_X[(i, j)] for j in range(num_sim_days)) for i in range(size)]
                        sim_max = sim_model.NewIntVar(0, num_sim_days, 'max')
                        sim_min = sim_model.NewIntVar(0, num_sim_days, 'min')
                        for s in sim_S:
                            sim_model.Add(sim_max >= s)
                            sim_model.Add(sim_min <= s)
                            
                        # סכימת כל ימי הבסיס של כולם בסימולציה
                        sim_total_days = sum(sim_X[(i, j)] for i in range(size) for j in range(num_sim_days))
                        
                        # פונקציית המטרה המשולבת
                        sim_model.Minimize((sim_max - sim_min) * 100 + sim_total_days)
                        
                        sim_solver = cp_model.CpSolver()
                        sim_solver.parameters.max_time_in_seconds = 5.0
                        
                        t0 = time.time()
                        sim_solver.Solve(sim_model)
                        t1 = time.time()
                        
                        times.append(t1 - t0)
                        
                    fig, ax = plt.subplots(figsize=(8, 4))
                    ax.plot(sizes, times, marker='o', linestyle='-', color='#556644', linewidth=2)
                    ax.set_title("Runtime Complexity (CP-SAT Solver)", fontsize=14, color='#1e2418')
                    ax.set_xlabel("Number of Soldiers (Input Size N)", fontsize=12)
                    ax.set_ylabel("Execution Time (Seconds)", fontsize=12)
                    ax.grid(True, linestyle='--', alpha=0.7)
                    
                    st.pyplot(fig)
                    st.success("גרף הביצועים הופק בהצלחה ומוכן לשילוב בדוח הסיכום או המצגת!")
